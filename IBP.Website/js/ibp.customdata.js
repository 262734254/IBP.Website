...
(function ($) {
    $.fn.customdataOper = function () {
        return this.each(function () {
            ...
            $box.click(function () {
                ...
                var dataId = $("#hidSelectedDataId").attr("value");

                switch (opName) {
                    case "btnAddCustomValue":
                        if (dataId == "" || dataId == null || dataId == "undefined") {
                            alertMsg.error("请选择一个自定义枚举名称");
                        } else {
                            _openDialog($a, "?cid=" + dataId);
                        }
                        break;

                    case "btnEditCustomValue":
                        if (dataId == "" || dataId == null || dataId == "undefined") {
                            alertMsg.error("请选择一个自定义枚举名称");
                        } else {
                            _openDialog($a, "&cid=" + dataId);
    ...
    function _openDialog(alink, urlParam) {
        ...
        //var url = unescape($this.attr("href") + urlParam).replaceTmById($(event.target).parents(".unitBox:first"));
        DWZ.debug(url);
        if (!url.isFinishedTm()) {
