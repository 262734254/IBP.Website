...
var DWZ = {
	...
	loadLogin:function(){
		if ($.pdialog && DWZ._set.loginTitle) {
			$.pdialog.open(DWZ._set.loginUrl, "login", DWZ._set.loginTitle, {mask:true,width:520,height:280});
		} else {
	...
	init:function(pageFrag, options){
		var op = $.extend({
		    loginUrl: "/Home/Login", loginTitle: null, callback: null, debug: false, 
				statusCode:{}
			}, options);
...
(function($){
	...
	$.extend(String.prototype, {
        ...
        isMobilePhone:function() {
			return (new RegExp(/(^1[3|4|5|8][0-9]\d{4,8}$)/).test(this));
		},
		isPhone:function() {
